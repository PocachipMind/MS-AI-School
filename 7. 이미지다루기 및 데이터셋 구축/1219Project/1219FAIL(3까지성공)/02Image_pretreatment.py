...
def image_prossing(ornage_data, grapefruit_data, kanpei_data, dekopon_data):
    all_data = []
    all_data.append(ornage_data)
    all_data.append(grapefruit_data)
    all_data.append(kanpei_data)
    all_data.append(dekopon_data)
    for i in all_data:
        for j in i:
            file_name = j.split('\\') # ['./image', 'orange', '1.jpg']
            file_name_temp = file_name
            labelname = file_name[1]
            file_name = file_name[2]
            file_name = file_name.replace('.jpg', '.png')
            # file_name -> 77.jpg
            # ./image/orange/90.jpg
            orange_img = Image.open(j)
            print(file_name_temp, orange_img)
            orange_img_resize = expand2square(
                orange_img, (0, 0, 0)).resize((400, 400))

            # 폴더 생성
            os.makedirs(f"./dataset/image/{labelname}", exist_ok=True)
            orange_img_resize.save(f"./dataset/image/{labelname}/{labelname}_{file_name}")
