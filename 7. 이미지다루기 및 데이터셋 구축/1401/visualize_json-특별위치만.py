...
def main():
    ...
    for use in ['valid', 'train']:
        ...
        seting_label = ['garbage_bag'] # 이미지 볼 폴더를 입력하세요

        for image_path in set_image_paths(image_root,seting_label):
...
def set_image_paths(root_path, seting_label):
    # root_path 경로 하위에 있는 모든 파일 경로 탐색
    paths = []
    for (path, dir, files) in os.walk(root_path):
        if os.path.basename(path) not in seting_label:
            continue
        for file in files:
            if file.split('.')[-1] != 'jpg':
                continue
