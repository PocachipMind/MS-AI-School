...
def main():
    ...
    for use in ['valid', 'train']:
        ...
        idx = 0
        while idx < len(get_image_paths(image_root)):
            ...
            # image, json matching
            image_path = get_image_paths(image_root)[idx]
            ...
            while True:
                key = cv2.waitKey()
                if key == ord('s'):
                    backup_path = os.path.join(backup_root, dirs[1], dirs[2], dirs[3], dirs[4], dirs[5])
                    os.makedirs(backup_path + '/image/', exist_ok=True)
                    os.makedirs(backup_path + '/label/', exist_ok=True)
                    shutil.move(image_path, backup_path + f'/image/{dirs[6]}')
                    shutil.move(label_path, backup_path + f'/label/{filename}')
                    print('File transfer complete >> ' + image_path)
                    break
                elif key == ord('q'):
                    idx -= 1
                    break
